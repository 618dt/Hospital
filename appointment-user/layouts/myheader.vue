  ...
  <div class="header-container">
    <div class="wrapper">
      ...
      <div class="left-wrapper v-link selected">
        <img style="width: 50px" width="50" height="50" src="~assets/images/logo0.png">
        <span class="text" onclick="javascript:window.location='/'">预约挂号统一平台</span>
      </div>
      <!-- 搜索框 -->
    ...
    <!-- 登录弹出层 如果css乱码增加v-if="dialogUserFormVisible"-->
...
export default {
  ...
  methods: {
    ...
    // 获取验证码 ^1[34578]\d{9}$ (手机号正则表达式)
    getCodeFun() {
      if (!(/\w[-\w.+]*@([A-Za-z0-9][-A-Za-z0-9]+\.)+[A-Za-z]{2,14}/.test(this.userInfo.phone))) {
      ...
      smsApi.sendCode(this.userInfo.phone).then(response => {
        this.redisCode = response.data //获取验证码
        this.timeDown();
      }).catch(e => {
    ...
    timeDown() {
      ...
      this.dialogAtrr.labelTips = '验证码'+this.redisCode+'已发送至' + this.userInfo.phone
